...
export const getOperationByClinicId = async (req: Request, res: Response) => {
    try {
        const operationsByClinicId = await operations.find({
...
export const getAllOperations = async (req: Request, res: Response) => {
    try {
        ...
        const getAllOperations = await operations.find({ deletedAt: null}).populate(['clinic']).sort({ createdAt: -1})
        if (getAllOperations) {
            res.json({
...
export const getOperation = async (req: Request, res: Response) => {
    try {
        const getOperation = await operations.findOne({ _id: req.params.id, deletedAt: null}).populate(['clinic'])
        if (getOperation) {
            res.json({
                item: getOperation
            })
    ...
    if (getOperation.length === 0) {
        try {
            const updateOperation = await operations.findByIdAndUpdate(
                req.params.id,{ $set: req.body, updatedAt: Date.now()},{ new: true}
            )
...
export const deleteOperation = async (req: Request, res: Response) => {
    try {
        ...
        if (getOperation.length > 0 ) {
            const deleteOperation = await operations.findByIdAndUpdate(req.params.id, { $set: { deletedAt: Date.now()}})
