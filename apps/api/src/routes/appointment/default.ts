...
export const getAllAppointments = async (req: Request, res: Response) => {
  try {
    ...
    const getAllAppointments = await appointment
      .find({ deletedAt: null })
      .populate(["procedure", "clinic"])
      .sort({
        createdAt: -1,
      });
...
export const getAppointment = async (req: Request, res: Response) => {
  try {
    const getAppointment = await appointment
...
export const addAppointment = async (req: Request, res: Response) => {
  ...
  if (
    (clinicId && procedureId && lastName && firstName && middleName) ||
    (email && contactNumber && date && time) ||
    declineReason
  ) {
    try {
