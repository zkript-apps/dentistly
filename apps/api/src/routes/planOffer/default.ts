...
try {
    const getAllPlanOffers = await planOffers.find({deletedAt:null}).sort({createdAt: -1})
    const planOffersCount = await planOffers.find({deletedAt:null}).countDocuments()
...
try {
    const getOfferByTableName = await planOffers.find({
        table:tableName,
        deletedAt:null
    }).sort({createdAt: -1})
    const offersCount = await planOffers.find({
        table:tableName,
        deletedAt:null
    }).countDocuments()
    if(offersCount!==0){
