...
export const getAllPatients = async (req: Request, res: Response) => {
    try {
      ...
      const getAllPatients = await patient.find({ deletedAt: null }).populate(['clinic']).sort({ createdAt: -1 })
      if(getAllPatients){
        res.json({
...
export const getPatient = async (req: Request, res: Response) => {
    try {
      const getPatient = await patient.findOne({ _id: req.params.id, deletedAt: null }).populate(['clinic'])
        if(getPatient){
          res.json({
          item: getPatient,
        }) 
  ...
  export const addPatient = async (req: Request, res: Response) => {
    const { clinicId, lastName, firstName, middleName, DoB, age, gender, address, status, relativesContactInfo } = req.body
    if (clinicId && lastName && firstName && middleName && DoB && age && gender && address && status && relativesContactInfo) {
  ...
  export const deletePatient = async (req: Request, res: Response) => {
    try {
      const getPatient = await patient.findOne({ _id: req.params.id, deletedAt: null });
      if (getPatient) {
