...
export const getAllProcedure = async (req: Request, res: Response) => {
    try {
        ...
        const getAllProcedure = await procedure.find({ deletedAt: null}).populate(['clinic']).sort({ createdAt: -1})
        if(getAllProcedure) {
            res.json({
...
export const getProcedure = async (req: Request, res: Response) => {
    try {
        const getProcedure = await procedure.findOne({ _id: req.params.id, deletedAt: null}).populate(['clinic'])
        if (getProcedure) {
            res.json({
            item: getProcedure,
            })
