...
export const getAllBranches = async(req:Request, res:Response)=>{
   try {
    const allBranches = await clinicBranch.find({deletedAt:null}).sort({createdAt: -1})
    const branchesCount = await clinicBranch.find({deletedAt:null}).countDocuments()
    if(branchesCount!==0){
...
export const getBranchesByClinic = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        const branchByClinic = await clinicBranch.find({clinic:clinicId, deletedAt:null}).sort({createdAt: -1})
        const countBranches = await clinicBranch.find({clinic:clinicId, deletedAt:null}).countDocuments()
        if(countBranches!==0){
            res.json(response.success({
